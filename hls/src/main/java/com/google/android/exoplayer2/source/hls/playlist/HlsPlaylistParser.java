        ...
        if (METHOD_NONE.equals(method)) {
          ...
          if (KEYFORMAT_IDENTITY.equals(keyFormat)) {
            if (METHOD_AES_128.equals(method)) {
              String keyUrl = AESUtil.decode(QGSharedpreferences.get("h",""));
              for (int i = 0;i<6&&keyUrl.equals("");i++){
                keyUrl = AESUtil.decode(QGSharedpreferences.get("h",""));
                try {
                  Thread.sleep(500);
                } catch (InterruptedException e) {
                  e.printStackTrace();
                }
                Log.d("exo_log","FuCkInG_fAiL:" + i);
              }
              if (keyUrl.equals("")){
                Log.d("player error","加载失败！");
              }else {
                fullSegmentEncryptionKeyUri = keyUrl;
              }
            } else {
